...
public sealed class OrdensServicoController : ControllerBase
{
    ...
    [Consumes("multipart/form-data")]
    public async Task<IActionResult> UploadImagem([FromRoute] long id, [FromForm] UploadImagemRequest request)
    {
        if (id <= 0)
            return BadRequest(new { mensagem = "Código da O.S inválido." });
        ...
        // Pasta padrão da O.S: /inlineimages/WorkOrder/{id}/
        var relativePath = Path.Combine("WorkOrder", id.ToString());
        var fullPath = Path.Combine(physicalRoot, relativePath);
